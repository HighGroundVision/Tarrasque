...
{
    public static class FnComboHandler
    {
        const int GAMEMODE_AD = 18;

        [StorageAccount("AzureWebJobsStorage")]
        [FunctionName("ComboHandler")]
        public static async Task Run(    
            // Queue {Name}
            [QueueTrigger("hgv-combos")]StatSnapshot item,
            // Stats for the Draft (Pre Seeded)
            [Blob("hgv-stats/combos/{Key}.json")]CloudBlockBlob statsBlob,
            // stats totals
        ...
        {
            //log.Info($"Fn-ComboHandler({item.Key}): started at {DateTime.UtcNow}");
        ...
        private static async Task CreateIfNoneExists(TraceWriter log, StatSnapshot item, CloudBlockBlob matchBlob)
        {
            try
            {
                log.Warning($"Fn-ComboHandler({item.Key}) No combo found; creating combo.");
            ...
            {
                log.Error($"Fn-ComboHandler({item.Key}): failed to create stats.", ex);
            }
        }
        ...
        private static async Task ProcessSnapshot(TraceWriter log, CloudBlockBlob matchBlob, StatSnapshot item, float totalMatches)
        {
            ...
            {
                log.Error($"Fn-ComboHandler({item.Key}): failed to update stats.", ex);
            }
        }
    }
}
